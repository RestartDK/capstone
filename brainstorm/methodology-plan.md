...
logEvent({ type: 'task_complete', taskId: 2, condition: 'ephemeral', success: true });
```
...
logEvent({ type: 'task_start', taskId: 2, condition: 'ephemeral', startTime: Date.now() });
// At task end
logEvent({ type: 'task_end', taskId: 2, condition: 'ephemeral', endTime: Date.now() });
```
