
# TODO

- [] Add this to the thesis:

> The thesis text should be revised so the final written method matches the implemented artifact.
>
> ### Update the methodology once the redesign is settled
>
> Revise the scenario descriptions so they match the actual participant actions.
>
> In particular, make sure:
>
> - the refinement scenario describes a real pre-commit refinement task
> - the task-execution scenario describes a real bounded workflow action
> - the trigger logic is described accurately
> - the variation allowed in the ephemeral condition is described as constrained and local
>
> ### Add a short justification for minimal background data
>
> State clearly that:
>
> - only analytically relevant participant variables are collected
> - personally identifying or non-essential demographic data is intentionally minimised
>
- [] Get 30 participants for my test
- [] Write my results for my thesis (while waiting u can work on naiss and take home)
- [] Download the data in csv and apply the analysis methods proposed in the methodology to then explain your actual findings on the subject
- [] Write about the discussion about the limitations and boundaries of the study
- [] Redo the methodolgy so it's inline with the final artifact with included pictures
- [] What changes because of my work here and how it can be applied in the real world
- [] Your conclusion of the project so what did i think in the end from what i did and what is next
