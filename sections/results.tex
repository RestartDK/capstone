\pagestyle{fancy}
\section{Results}
\input{sections/generated-result-values}
...
A total of \GenNTotal{} participants accessed the study website. Of these, \GenNValid{} completed both task trials and the final questionnaire and are included in the analysis. The remaining participants were excluded because they did not finish both conditions or did not complete the post-study questions. The reported results therefore describe the completer sample rather than all users who initially opened the study website. Table~\ref{tab:demographics} summarises the background characteristics of the included sample.
...
Completion time was measured from the moment the participant started the trial to the moment they submitted their answer. Participants who completed the task in the baseline condition had a median time of \GenTimeBaselineMdn{}~seconds (IQR: \GenTimeBaselineIqr{}), compared to \GenTimeEphemeralMdn{}~seconds (IQR: \GenTimeEphemeralIqr{}) in the ephemeral condition. A Wilcoxon signed-rank test \GenTimeWilcoxonResult{} ($W = \GenTimeW$, $p = \GenTimeP$, $r = \GenTimeR$). Although the ephemeral median was slightly higher, the result does not indicate a reliable difference in efficiency between conditions.
...
The total number of logged interactions per trial was used as a proxy for task effort. The median interaction count in the baseline condition was \GenIntBaselineMdn{} (IQR: \GenIntBaselineIqr{}), compared to \GenIntEphemeralMdn{} (IQR: \GenIntEphemeralIqr{}) in the ephemeral condition. A Wilcoxon signed-rank test \GenIntWilcoxonResult{} ($W = \GenIntW$, $r = \GenIntR$; $p$ = \GenIntP{}).

This higher count should be interpreted with caution because the ephemeral condition includes support-related events such as requesting support, support being shown, dismissal, and detail expansion. The increase therefore reflects greater overall interface interaction, not only additional effort spent on the underlying slide-reordering task itself.
...
The ephemeral condition received a higher median helpfulness rating than the baseline (\GenLikertHelpfulnessEphemeralMdn{} vs. \GenLikertHelpfulnessBaselineMdn{}), but the Wilcoxon test did not reach the conventional threshold for statistical significance ($p = \GenLikertHelpfulnessP{}$). This suggests a positive directional trend in perceived usefulness without strong enough evidence to claim a reliable effect in this sample.

Intrusiveness ratings were higher in the ephemeral condition (median \GenLikertIntrusivenessEphemeralMdn{}) than in the baseline condition (median \GenLikertIntrusivenessBaselineMdn{}), and this difference was statistically significant ($p = \GenLikertIntrusivenessP{}$). Participants therefore experienced the temporary support as more noticeable or interruptive than the baseline interface.

Perceived control had the same median in both conditions (\GenLikertControlBaselineMdn{} and \GenLikertControlEphemeralMdn{}), and the paired comparison was not significant ($p = \GenLikertControlP{}$). Within the bounds of this study, the ephemeral support did not measurably reduce participants' sense of control over the task.
...
Excluding the \GenPrefOverallN{} participants who expressed no preference, \GenPrefOverallE{} participants preferred the ephemeral condition and \GenPrefOverallB{} preferred the baseline. A binomial test did not show a statistically significant overall preference difference ($p = \GenPrefOverallBinomialP{}$), indicating that the sample did not meaningfully favour one condition over the other.
...
Of the \GenNValid{} participants who completed the ephemeral trial, support was triggered or explicitly requested in \GenEngNTriggered{} cases (\GenEngPctTriggered{}\%). At the participant level, \GenEngNUsed{} participants recorded at least one \texttt{support\_used} event, \GenEngNDismissed{} recorded at least one \texttt{support\_dismissed} event, \GenEngNIgnored{} recorded at least one \texttt{support\_ignored} event, and \GenEngNExpanded{} recorded at least one detail-expansion event. These categories are not mutually exclusive: a participant could, for example, inspect or use support and later dismiss it in the same trial. The support generation pipeline produced \GenEngTotalGen{} specifications in total, which are treated in the present analysis as model-generated outputs rather than deterministic fallbacks.
