...
\begin{figure}[htbp]
    ...
    \fbox{%
        ...
        \textit{[Diagram placeholder.]}\quad
        Three-tier block diagram: browser client (React, Tailwind, \texttt{@dnd-kit}, ephemeral overlay) $\rightarrow$ Next.js API (participants, trials, events, support, questionnaires) $\rightarrow$ PostgreSQL and Google Gemini (structured output). Arrows indicate data flow.
        \end{minipage}%
    }
...
\begin{figure}[htbp]
    ...
    \fbox{%
        \begin{minipage}{0.94\textwidth}\footnotesize
        \textit{[Diagram placeholder.]}\quad
        Sequential flow: Landing $\rightarrow$ Consent $\rightarrow$ Background $\rightarrow$ Trial~1 $\rightarrow$ Post-trial~1 $\rightarrow$ Trial~2 $\rightarrow$ Post-trial~2 $\rightarrow$ Final questionnaire $\rightarrow$ Completion. Annotate \texttt{resolveStudyState} and incremental trial creation.
        \end{minipage}%
    }
    \caption{Participant flow through the experimental website. Each step is enforced by a server-side state machine that prevents skipping or reordering.}
...
\begin{figure}[htbp]
    ...
    \fbox{%
        \begin{minipage}{0.94\textwidth}\footnotesize
        \textit{[Diagram placeholder.]}\quad
        Expanded pipeline: task state and snapshot $\rightarrow$ prompts $\rightarrow$ Gemini structured output $\rightarrow$ validation (envelope, node tree, props, target allowlist, structural rules, ancestry, HTML sanitisation) $\rightarrow$ accept or fallback $\rightarrow$ persist to \texttt{support\_outputs} $\rightarrow$ render overlay.
        \end{minipage}%
    }
