...
import { db, participants, questionnaireResponses, supportOutputs, trialEvents, trials } from "./db";
...
function rowsToCsv(headers: string[], rows: Record<string, string | number | null>[]): string {
...
export async function exportStudyCsvs(): Promise<{
  ...
  const pRows = await db.select().from(participants).orderBy(asc(participants.createdAt));
  ...
  const qRows = await db.select().from(questionnaireResponses).orderBy(asc(questionnaireResponses.id));
  const qCsv = rowsToCsv(
    ["id", "participant_id", "trial_id", "created_at", "question_key", "response_value"],
    qRows.map((q) => ({
      id: q.id,
